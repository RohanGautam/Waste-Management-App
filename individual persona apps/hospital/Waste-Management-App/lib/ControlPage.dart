...
void hospitalLockSend(
    {weight = '223.89',
    volume = '25000.00',
    tId = '123432',
    sId = 'NPPj78iIYPeI0J8Ji1aqUK8ETAo2',
    dID = '97820',
    gf_x = '12.98',
    gf_y = '89.01',
    gf_rad = '5.00'}) async {
  ...
  databaseReference
      .child('BIN_STATUS')
      .child("ON_JOB")
      .child("BINID_667_${firebaseCounter}")
      .update({
...
class _ControlPageState extends State<ControlPage> {
      ...
      for (int i = 0; i < optionLocations.length; i++) {
        ...
        markers[MarkerId('${i + 1}')] = Marker(
          ...
          infoWindow: InfoWindow(
              title: "Location ${i + 1}",
              snippet: 'Location of falility ${i + 1}'),
        );
      }
    });
  ...
  void onReportPressed() {
    ...
    Navigator.of(context).push(MaterialPageRoute(builder: (context) {
      return AfterLockReport(
          weight: temp['weight'],
          volume: temp['volume'],
          destination: _dropdownvalue,
          src: "Hospital #232",
          latlng:
              LatLng(double.parse(temp['loc_x']), double.parse(temp['loc_y'])));
        ...
        : isConnected
            ? () async {
                ...
                hospitalLockSend(
                    ...
                    gf_x: parsedData['loc_x'],
                    gf_y: parsedData['loc_y'],
                    gf_rad: parsedData['radius']);
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            customButton("${widget.persona} ${lock ? "lock" : "unlock"}",
                func: (lock ? hospitalLock : hospitalUnlock)),
  ...
  parseDataRecieved(String dataRecieved) {
    ...
    String data = dataRecieved.substring(
        dataRecieved.indexOf("#") + 1, dataRecieved.length);
    if (type == "LOCK") {
